...
def res_block(
        input_tensor,
        filters,
        strides=RES_STRIDES_LIST_DEFAULT,  # Default: (2, 1)
        pool_size=RES_POOL_SIZE_DEFAULT,    # Default: 2
...
def save_keras_model(model,
  ...
  """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        tf.io.gfile.RecursivelyCopyDir(temp_dir, path, overwrite=True)
...
def save_keras_model_as_h5(model,
  ...
  Args:
    ...
    include_optimizer: whether the optimizer should also be saved.
  """
    file_util.maybe_make_dirs(os.path.dirname(filename))
    with tempfile.NamedTemporaryFile(suffix='.h5', delete=False) as f:
        temp_filepath = f.name
        tf.keras.models.save_model(model, temp_filepath, include_optimizer=include_optimizer)
        tf.io.gfile.copy(f.name, filename, overwrite=True)


def save_dict_to_json(data, path):
    """Saves dict as JSON file."""
    json_str = json.dumps(data, indent=2) + '\n'
    file_util.maybe_make_dirs(os.path.dirname(path))
