...
def get_features_dict(sample_size, features):
    ...
    columns = [tf.io.FixedLenFeature(shape=sample_shape, dtype=tf.float32)] * len(features)
...
def _validate_input_features(input_features=constants.INPUT_FEATURES):
...
def _validate_output_features(output_features=constants.OUTPUT_FEATURES):
    """Validates values for `output_features`."""
    if not all(x in constants.OUTPUT_FEATURES for x in output_features):
        raise ValueError(f'output_features=[{output_features}] should be present in '
...
def _parse_journal2021_dataset(
    ...
    if input_sequence_length > 1:
        inputs_stacked = tf.reshape(inputs_stacked, [-1, num_in_channels, data_size, data_size])
    ...
    if output_sequence_length > 1:
        ...
        if len(outputs_stacked_shape) != 4:
            raise ValueError(f'outputs_stacked should be rank 4 but dimensions are {outputs_stacked_shape}')
        output_img = tf.transpose(outputs_stacked, [0, 2, 3, 1])
    else:
        outputs_stacked_shape = outputs_stacked.get_shape().as_list()
        if len(outputs_stacked_shape) != 3:
            raise ValueError(f'outputs_stacked should be rank 3 but dimensions are {outputs_stacked_shape}')
...
def _parse_fn(
        example_proto, input_sequence_length, output_sequence_length, data_size,
        sample_size, output_sample_size, downsample_threshold, binarize_output,
        input_features, output_features, clip_and_normalize, clip_and_rescale,
        random_flip, random_rotate, random_crop, center_crop, azimuth_in_channel,
        azimuth_out_channel):
    ...
    output_img = image_utils.downsample_output_image(
        output_img, output_sample_size, downsample_threshold, binarize_output)
    return input_img, output_img


def get_dataset(
        file_pattern, data_size, sample_size, output_sample_size, batch_size,
        input_features, output_features, shuffle, shuffle_buffer_size,
        compression_type, input_sequence_length, output_sequence_length, repeat,
        clip_and_normalize, clip_and_rescale, random_flip, random_rotate,
        random_crop, center_crop, azimuth_in_channel, azimuth_out_channel,
        downsample_threshold=0.0, binarize_output=True):
    ...
    dataset = dataset.map(
        lambda x: _parse_fn(
            ...
            sample_size, output_sample_size, downsample_threshold, binarize_output,
            input_features, output_features, clip_and_normalize, clip_and_rescale,
            random_flip, random_rotate, random_crop, center_crop,
            azimuth_in_channel, azimuth_out_channel),
        num_parallel_calls=tf.data.experimental.AUTOTUNE)
    if shuffle:
...
def make_dataset(hparams, mode="train"):
    """Creates a dataset."""
    ...
    if (not hparams.azimuth_in_channel or hparams.azimuth_in_channel not in input_features):
        azimuth_in_channel = None
    else:
        azimuth_in_channel = input_features.index(hparams.azimuth_in_channel)

    if (not hparams.azimuth_out_channel or hparams.azimuth_out_channel not in output_features):
